...
mod tests {
    ...
    fn neutral_g1_pose() -> [Mat4; 34] {
        let clip = load_g1_frames(&format!("{}/mb_idle.g1", assets_dir()))
            .expect("mb_idle.g1 must be present for neutral pose tests");
        clip.into_iter()
            .next()
            .expect("mb_idle.g1 must contain at least one frame")
    }
