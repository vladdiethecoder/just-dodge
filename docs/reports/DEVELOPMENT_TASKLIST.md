...
| B.1.3 | M3-MOTION-RETARGET — Retarget to armored 24-bone C0 | `src/retarget.rs`, `src/asset.rs`, `src/main.rs`, `src/bin/shot.rs` | Create 24-bone MotionBricks-to-armored-duelist mapping. Replace `App::current_pose()` bind-pose with retargeted motion output. Verify root alignment, bone orientation, weapon-hand socket. | No non-finite matrices, mesh collapse, limb inversion, root drift. Bind-pose regression valid. Identical replay → identical pose receipt hashes. | B.1.2 |
